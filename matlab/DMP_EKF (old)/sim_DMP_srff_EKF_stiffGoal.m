...
for n=1:1
    ...
    while (true)
        ...
        %Ky = 10*[ 0   4   5   6  8    10   20   30  40];
        %Ky = 4*[ 2   4   5   6  8    10   20   30  40];
        Ky = 2*[ 2   4   5   6  8    10   20   30  40];
        for i=1:length(Kx)
            if (kx <= Kx(i))
                if (i==length(Kx))
                    k_i = Ky(i);
        ...
        F_ext = inv_M_rh*(-ddy_hat) + inv_M_rh*( inv_M_r*(D_r*(dy_r-dy_hat)+K_r*(y_r-y_hat)) - inv_M_h*(D_h*dy_h+K_h*(y_h-g)) );
        ...
        ddy_h = -inv_M_h*(D_h*(dy_h) + K_h*(y_h-g) + F_ext);
        
        if (norm(ddy_h-ddy_r)>1e-12)
            bond_err = norm(ddy_h-ddy_r);
            warning(['t=' num2str(t) ' sec: Error:' num2str(bond_err) ': Rigid bond contraint might be violated!']);
        end
