...
for n=1:1
    ...
    Q_w = eye(N_params, N_params)*0.0;
    R_v = eye(N_out, N_out)*0.01;
    
    %% === ukf ===
    ukf = unscentedKalmanFilter(@(theta)stateTransFun(theta), @(theta, msr_cookie)msrFun(theta, msr_cookie), theta, 'HasAdditiveMeasurementNoise',true);
    ukf.State = theta;
    ukf.StateCovariance = P_theta;
    ukf.ProcessNoise = Q_w;
    ukf.MeasurementNoise = R_v;
    ukf.Alpha = 1e-3;
    ukf.Beta = 2;
    ukf.Kappa = 0;
    ...
    disp('DMP-UKF matlab simulation...')
    tic
    while (true)
